...
test(`Page object model happy path for second test`, async ({ page }): Promise<void> => {
    ...
    await doesEmployeeWorkPage.selectRadioOption(page, doesEmployeeWorkPage_content.radioText_no);
    await doesEmployeeWorkPage.continueOn(page);
    ...
    await holidayEntitlementPage.selectRadioOption(page, holidayEntitlementPage_content.radioText_daysPerWeek);
    ...
    await cyaPage.checkPage(page, '28 days');
});
